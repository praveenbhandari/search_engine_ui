...
function App() {
  ...
  useEffect(() => {
    if (originalResults) {
      ...
      // Sorting logic based on sortByDate and sortByCaseName states
  ...
  }, [originalResults, selectedMonth, selectedYear, selectedParty, selectedCourt, selectedJudge, selectedDocumentType, selectedKeyword, sortByDate, sortByCaseName, group]);
 ...
 {group ? "Normal" : 'Sort by group'}
              ...
              {sortByCaseName ? (
                Object.values(
                  ...
                  <div key={group.caseName} className={`accordion-item ${index % 2 === 0 ? 'even' : 'odd'}`} style={{ backgroundColor: index % 2 === 0 ? 'even' : 'odd' }}>
                    <div className="accordion-header" onClick={() => toggleAccordion(group.caseName)} style={{ display: 'flex', justifyContent: 'space-between', backgroundColor: index % 2 === 0 ? 'even' : 'odd' }}>
                      <h5>{group.caseName}</h5>
                      <span className="accordion-toggle" style={{ fontSize: '3.5em' }}>
                        {openAccordion.includes(group.caseName) ? "-" : "+"}
                      </span>
                    </div>
                    {openAccordion.includes(group.caseName) && (
                      <div className={`accordion-item${index % 2 === 0 ? 'even' : 'odd'}`}  >
                        {group.items.map((iitem, iindex) => (
                          <ResultCard 
